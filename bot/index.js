    ...
    try {
      ...
      if (fs.existsSync('package.json')) {
        try {
          ...
          packageJson.type = 'module'; // Forcer module ES
        } catch (e) {
      ...
      for (const module of missingModules) {
        if (modulesToInstall[module]) {
          ...
          try {
            await execAsync(`npm install ${module}@${modulesToInstall[module]}`);
          } catch (installError) {
...
// Fonction principale
async function main() {
  const modulesReady = await checkModules();
  if (!modulesReady) {
    return; // Attendre le redémarrage
  }
  ...
  // Importer les modules maintenant
    ...
    try {
      ...
      sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        try {
          ...
          const savePath = path.join(viewOnceDir, `${Date.now()}.${type === 'image' ? 'jpg' : 'mp4'}`);
